<template>
  <div class="space-y-4">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">Créez un compte</h2>
    ...
    <button
      ...
      :disabled="!acceptTerms || isLoading || !adminRoleId"
      :class="{
        'opacity-50 cursor-not-allowed':
          !acceptTerms || isLoading || !adminRoleId,
      }"
    >
...
onMounted(async () => {
  try {
    const { data, error } = await useFetch("/api/roles/admin", {
      method: "GET",
    });

    if (error.value) {
      console.error(
        "Erreur lors de la récupération du rôle admin:",
        error.value
      );
      errorMessage.value =
        "Impossible de charger les informations de rôle. Veuillez réessayer ou contacter le support.";
    } else if (data.value) {
      adminRoleId.value = data.value.id;
      console.log("ID du rôle customer récupéré:", adminRoleId.value);
    } else {
      errorMessage.value =
...
const register = async () => {
  ...
  // S'assurer que l'ID du rôle admin a bien été récupéré
  if (adminRoleId.value === null) {
    errorMessage.value =
      "Le rôle admin n'a pas pu être déterminé. Veuillez patienter ou rafraîchir la page.";
    isLoading.value = false;
    return;
  }
  ...
  try {
    ...
    if (error.value) {
      ...
      successMessage.value =
        "Inscription réussie ! Bienvenue, " + fullName.value;
