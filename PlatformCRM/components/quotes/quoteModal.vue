<template>
  ...
  >
    <div class="bg-white rounded-lg w-full max-w-2xl mx-4 sm:m">
      <div class="p-6">
        ...
        <form @submit.prevent="createQuote" class="space-y-4">
          ...
          <div class="flex justify-end space-x-4">
            <button
              type="button"
              @click="saveAsDraft"
              class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-all whitespace-nowrap cursor-pointer !rounded-button"
            >
...
const createQuote = () => {
  emit('close'); // Fermer le modal après la création (ou après l'action souhaitée)
};
const saveAsDraft = () => {
  // Implementation for saving as draft
  emit('close'); // Fermer le modal après l'enregistrement (ou après l'action souhaitée)
};
</script>
