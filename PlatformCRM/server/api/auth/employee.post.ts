...
export default defineEventHandler(async (event) => {
    try {
        ...
        const { nom, email, password, poste, department } = body;
        ...
        // 1. Rechercher l'ID du rôle "employee"
        ...
        if (!employeeRole) {
            ...
            throw createError({
                statusCode: 500,
                message: 'Le rôle "employee" est introuvable. Veuillez contacter l\'administrateur.',
            });
        }
        ...
        return {
            statusCode: 201, 
            message: 'Utilisateur employee créé avec succès !',
            user: {
                id: newUser.id,
                name: newUser.name,
                email: newUser.email,
        ...
        console.error('Erreur lors de la création de l\'utilisateur employee:', error);
