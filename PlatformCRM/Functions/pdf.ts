...
export async function generateInvoicePdf(invoiceId: number) {
  try {
    ...
    const outputPath = `facture_${invoice.invoiceNumber}.pdf`;
    ...
    const clientAddress = invoice.quote?.customer?.adresse || invoice.User?.adresse || "Adresse Inconnue";
    ...
    doc.fillColor(textColor)
       .text(`Sous-total: ${calculatedSubtotal.toFixed(2)} CFA`, { align: "right" });
    doc.moveDown(0.2);
